...
resource "aws_lambda_function" "upload-lambda" {
  ...
  handler          = "pennsieve_upload_service_v2_upload"
  runtime          = "go1.x"
  ...
  filename         = "${path.module}/../lambda/bin/pennsieve_upload_service_v2_upload.zip"

  vpc_config {
    subnet_ids         = tolist(data.terraform_remote_state.vpc.outputs.private_subnet_ids)
    security_group_ids = [aws_security_group.upload_v2_service_security_group.id]
  }
...
data "archive_file" "upload_trigger_lambda_archive" {
  ...
  output_path = "${path.module}/../lambda/bin/pennsieve_upload_service_v2_upload.zip"
}