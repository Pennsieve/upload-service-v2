resource "aws_apigatewayv2_api" "upload_service_api" {
  name          = "Upload Manifest API"
  protocol_type = "HTTP"
...
resource "aws_apigatewayv2_api_mapping" "upload_service_api_map" {
...
resource "aws_apigatewayv2_stage" "upload_service_gateway_stage" {
  api_id = aws_apigatewayv2_api.upload_service_api.id
  ...
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.upload_service_gateway_log_group.arn
...
resource "aws_apigatewayv2_integration" "upload_service_integration" {
  api_id             = aws_apigatewayv2_api.upload_service_api.id
...
resource "aws_lambda_permission" "accounts_service_lambda_permission" {
  ...
  source_arn = "${aws_apigatewayv2_api.upload_service_api.execution_arn}/*/*"
}